...
} = useTicketsTrend(fromRef, toRef);
const {
  data: agentData,
  isLoading: agentLoading,
  refetch: refetchAgent,
} = useAgentPerformance(fromRef, toRef);
const {
  data: clientData,
  isLoading: clientLoading,
  refetch: refetchClients,
} = useTopClients(fromRef, toRef, 10);
const {
  data: slaData,
  isLoading: slaLoading,
  refetch: refetchSla,
} = useSlaTrend(fromRef, toRef);
...
const slaSeries = computed(() => [
  ...
  {
    name: t("reports.slaBreached"),
    data: (slaData.value ?? []).map((p) => p.breached),
  },
]);
...
const clientsOptions = computed<ApexOptions>(() => ({
  ...
  dataLabels: {
    enabled: true,
    style: { fontFamily: "inherit", fontWeight: 600, fontSize: "11px" },
  },
  tooltip: { theme: "dark" },
  grid: { borderColor: "var(--border, #e5e7eb)" },
  noData: {
  ...
  <div class="p-6 space-y-6">
    ...
    <div class="flex items-center justify-between flex-wrap gap-3">
      ...
      <div class="flex items-center gap-2">
        <!-- Range selector -->
        <div
          class="flex rounded-[var(--radius)] border border-[var(--border)] overflow-hidden text-sm"
        >
          <button
    ...
    <div class="grid grid-cols-1 xl:grid-cols-2 gap-6">
      ...
      <Card>
        ...
        >
          ...
          <Column
            field="assigned"
            :header="t('reports.assigned')"
            class="text-center"
          />
          <Column
            field="resolved"
            :header="t('reports.resolved')"
            class="text-center"
          />
          <Column :header="t('reports.avgTime')">
            <template #body="{ data }">
              <span v-if="data.avgMinutes !== null">{{
                formatMinutes(data.avgMinutes)
              }}</span>
