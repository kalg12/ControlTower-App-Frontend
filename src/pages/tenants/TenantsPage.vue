...
const onEditSubmit = editForm.handleSubmit(async (values) => {
  ...
  try {
    await tenantsService.update(editingTenant.value.id, {
      name: values.name,
      slug: values.slug || undefined,
      country: values.country,
      timezone: values.timezone,
  ...
  confirm.require({
    message:
      action === "suspend"
        ...
        : t("tenants.reactivateConfirm", { name: tenant.name }),
    header:
      action === "suspend"
        ? t("tenants.suspendTitle")
        : t("tenants.reactivateTitle"),
    icon: "pi pi-exclamation-triangle",
    rejectProps: {
      label: t("common.cancel"),
      severity: "secondary",
    ...
    acceptProps: {
      label:
        action === "suspend" ? t("common.suspend") : t("common.reactivate"),
      severity: action === "suspend" ? "warn" : "success",
    },
    accept: async () => {
      try {
        if (action === "suspend") {
        ...
        toast.error(
          action === "suspend"
            ? t("tenants.suspendFailed")
            : t("tenants.reactivateFailed"),
        );
      }
    },
  });
...
<template>
      ...
      <div class="flex items-center gap-2">
        <div>
          ...
          <p class="text-sm text-[var(--text-muted)]">
            {{ t("tenants.totalCount", { count: totalRecords }) }}
          </p>
        </div>
        <PageInfoButton
          :title="t('tenants.title')"
          :description="t('pageInfo.tenants')"
        />
      </div>
            ...
            :severity="
              row.status === 'ACTIVE'
                ...
                : row.status === 'SUSPENDED'
                  ...
                  : 'warn'
            "
            :value="tenantStatusLabel(row.status)"
          />
        </template>
      </Column>
      <Column
      ...
      <Column :header="t('common.actions')" style="width: 130px">
        <template #body="{ data: row }: { data: Tenant }">
          <div class="flex gap-1">
            ...
            />
            <Button
              :icon="row.status === 'SUSPENDED' ? 'pi pi-play' : 'pi pi-pause'"
              :severity="row.status === 'SUSPENDED' ? 'success' : 'warn'"
              text
              rounded
              size="small"
              v-tooltip.top="
    ...
    <form class="flex flex-col gap-4" @submit.prevent="onCreateSubmit">
      ...
      >
        <InputText
          v-model="cName"
          v-bind="cNameAttrs"
          :placeholder="t('tenants.formNamePlaceholder')"
          class="w-full"
          :disabled="isSubmitting"
        />
      </FormField>
      <FormField
        :label="t('tenants.formSlug')"
        name="c-slug"
        :error="createForm.errors.value.slug"
      >
        <InputText
          v-model="cSlug"
          v-bind="cSlugAttrs"
          :placeholder="t('tenants.formSlugPlaceholder')"
          class="w-full"
          :disabled="isSubmitting"
        />
      </FormField>
  ...
  <AppDialog
    ...
    :subtitle="t('tenants.editSubtitle')"
  ...
  >
    <form class="flex flex-col gap-4" @submit.prevent="onEditSubmit">
      ...
      <FormField
        :label="t('tenants.formSlug')"
        name="e-slug"
        :error="editForm.errors.value.slug"
      >
        <InputText
          v-model="eSlug"
          v-bind="eSlugAttrs"
