      ...
      <div class="flex items-center gap-2">
        <div>
          ...
          <p class="text-sm text-[var(--text-muted)]">
            {{ t("audit.totalCount", { count: totalRecords }) }}
          </p>
        </div>
        <PageInfoButton
          :title="t('audit.title')"
          :description="t('pageInfo.audit')"
        />
      </div>
      <Button
        icon="pi pi-refresh"
    ...
    >
      ...
      >
        <template #body="{ data: row }: { data: AuditLog }">
          <span class="text-[var(--text-muted)] font-mono text-xs">{{
      ...
      <Column field="result" :header="t('audit.result')" style="width: 110px">
        <template #body="{ data: row }: { data: AuditLog }">
          <Tag
            v-if="row.result"
            :severity="resultSeverity(row.result)"
            :value="row.result"
            class="text-xs"
          />
      ...
      <template #expansion="{ data: row }: { data: AuditLog }">
        <div class="px-4 py-3 space-y-3 bg-[var(--surface)] text-sm">
          <div v-if="row.resourceId" class="flex gap-2">
            <span class="font-medium text-[var(--text-muted)] w-28 shrink-0">{{
              t("audit.resourceId")
            }}</span>
            <span class="font-mono text-xs text-[var(--text)] break-all">{{
              row.resourceId
            }}</span>
          </div>
          <div v-if="row.oldValue" class="flex gap-2">
            ...
            <pre
              class="font-mono text-xs text-orange-600 dark:text-orange-400 whitespace-pre-wrap break-all max-h-40 overflow-auto"
              >{{ tryFormatJson(row.oldValue) }}</pre
            >
          </div>
          <div v-if="row.newValue" class="flex gap-2">
            ...
            <pre
              class="font-mono text-xs text-green-600 dark:text-green-400 whitespace-pre-wrap break-all max-h-40 overflow-auto"
              >{{ tryFormatJson(row.newValue) }}</pre
            >
          </div>
          <div
            v-if="!row.resourceId && !row.oldValue && !row.newValue"
