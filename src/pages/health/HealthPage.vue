...
const incidentTotalElements = computed(
  () => incidentLogPage.value?.totalElements ?? 0,
);
...
const canWriteIntegrations = computed(() =>
  authStore.hasPermission("integration:write"),
);
...
const {
  ...
  errors: posErrors,
  resetForm: resetPosForm,
  defineField: definePosField,
} = useForm({
...
function confirmTogglePosEndpoint(ep: Integration) {
  ...
  confirm.require({
    message: activating
      ? t("health.activateConfirm")
      : t("health.deactivateConfirm"),
    header: activating
      ? t("health.activateTitle")
      : t("health.deactivateTitle"),
    ...
    acceptProps: {
      label: activating ? t("health.activate") : t("health.deactivate"),
    ...
    accept: async () => {
      try {
        ...
        toast.success(
          activating
            ? t("health.activateSuccess")
            : t("health.deactivateSuccess"),
        );
      } catch {
...
const branchOptions = computed(() => {
  const base = items.value.map((c) => ({
    label: c.branchName ?? c.branchId,
    value: c.branchId,
  }));
...
<template>
    ...
    <template v-else>
      ...
      <div class="grid grid-cols-3 gap-4">
        ...
        <Card>
          <div class="flex items-center gap-3">
            <div
              class="w-9 h-9 rounded-lg bg-red-50 dark:bg-red-950 flex items-center justify-center"
            >
      ...
      >
          ...
          <template #body="{ data: row }: { data: HealthCheck }">
            <div>
              <span class="font-medium text-[var(--text)]">{{
                row.branchName ?? t("common.none")
              }}</span>
              <span class="block text-xs text-[var(--text-muted)] font-mono">{{
                row.branchId
              }}</span>
              <span
        ...
        <Column
          field="version"
          :header="t('health.version')"
          style="width: 110px"
        >
          <template #body="{ data: row }: { data: HealthCheck }">
            <span class="text-[var(--text-muted)] text-sm font-mono">{{
              row.version ?? t("common.none")
            }}</span>
          </template>
        </Column>
      ...
      <div>
        <div class="flex items-center justify-between mb-3 flex-wrap gap-2">
          <div>
            <h3 class="text-base font-semibold text-[var(--text)]">
              {{ t("health.incidentLog") }}
            </h3>
            <p class="text-xs text-[var(--text-muted)]">
              {{ t("health.incidentLogSubtitle") }}
            </p>
          </div>
          <div class="flex items-center gap-2">
            <!-- Branch filter -->
        ...
        <template v-else>
          ...
          >
            ...
            >
              <template #body="{ data: row }: { data: HealthIncident }">
                <div>
                  ...
                  <span
                    class="block text-xs text-[var(--text-muted)] font-mono truncate max-w-[140px]"
                    :title="row.branchId"
                    >{{ row.branchId }}</span
                  >
                </div>
            ...
            <Column
              field="description"
              :header="t('health.description')"
              style="min-width: 220px"
            >
              <template #body="{ data: row }: { data: HealthIncident }">
                <span class="text-sm text-[var(--text)] break-words">{{
                  row.description
                }}</span>
              </template>
            </Column>
            ...
            >
              <template #body="{ data: row }: { data: HealthIncident }">
                <span
                  ...
                  :class="
                    row.open ? 'text-red-500' : 'text-[var(--text-muted)]'
                  "
                >
      ...
      <div>
        <div class="flex items-center justify-between mb-3">
          <div>
            <h3 class="text-base font-semibold text-[var(--text)]">
              {{ t("health.monitoredEndpoints") }}
            </h3>
            <p class="text-xs text-[var(--text-muted)]">
              {{ t("health.endpointsSubtitle") }}
            </p>
          </div>
        ...
        <template v-else>
          ...
          >
            <Column
              field="clientBranchId"
              :header="t('health.branchId')"
            ...
            <Column
              ...
              :header="t('health.interval')"
              sortable
              style="width: 100px"
            >
              <template #body="{ data: row }: { data: Integration }">
                <span class="text-sm text-[var(--text-muted)]"
                  >{{ row.heartbeatIntervalSeconds }}s</span
                >
              </template>
            </Column>
            ...
            >
              <template #body="{ data: row }: { data: Integration }">
                <Tag
                  ...
                  :value="endpointStatusLabel(row.active)"
                />
              </template>
            </Column>
              ...
              <template #body="{ data: row }: { data: Integration }">
                <div class="flex gap-1">
                  ...
                  <Button
                    ...
                    size="small"
                    v-tooltip.top="
                      row.active ? t('health.deactivate') : t('health.activate')
                    "
  ...
  >
    <form class="flex flex-col gap-4" @submit.prevent="onSubmitPos">
      <FormField
        :label="t('health.formClient')"
        name="clientId"
        :error="posErrors.clientId"
        required
      >
        <Select
      ...
      <FormField
        :label="t('health.formBranch')"
        name="clientBranchId"
        :error="posErrors.clientBranchId"
        required
      >
        <Select
      ...
      <FormField
        :label="t('health.formPullUrl')"
        name="pullUrl"
        :error="posErrors.pullUrl"
        required
      >
        <InputText
      ...
      <FormField
        :label="t('health.formApiKey')"
        name="apiKey"
        :error="posErrors.apiKey"
      >
        <InputText
          v-model="apiKeyValue"
      ...
      <FormField
        :label="t('health.formInterval')"
        name="heartbeatIntervalSeconds"
        :error="posErrors.heartbeatIntervalSeconds"
      >
        <InputNumber
