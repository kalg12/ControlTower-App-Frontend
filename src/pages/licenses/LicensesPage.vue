...
function confirmSuspend(license: License) {
  ...
  confirm.require({
    ...
    header:
      license.status === "SUSPENDED"
        ? t("licenses.reactivateTitle")
        : t("licenses.suspendTitle"),
    ...
    acceptProps: {
      label:
        license.status === "SUSPENDED"
          ? t("common.reactivate")
          : t("common.suspend"),
      severity: license.status === "SUSPENDED" ? "success" : "warn",
    },
    accept: async () => {
      try {
        ...
        toast.error(
          license.status === "SUSPENDED"
            ? t("licenses.reactivateFailed")
            : t("licenses.suspendFailed"),
        );
      }
    },
  });
...
function confirmCancel(license: License) {
  confirm.require({
    message: t("licenses.cancelConfirm", {
      client: license.clientName || license.id,
    }),
    header: t("licenses.cancelTitle"),
    icon: "pi pi-exclamation-triangle",
    rejectProps: {
      label: t("licenses.keepLicense"),
      severity: "secondary",
      outlined: true,
    },
      ...
      <div class="flex items-center gap-2">
        <div>
          ...
          <p class="text-sm text-[var(--text-muted)]">
            {{ t("licenses.totalCount", { count: totalRecords }) }}
          </p>
        </div>
        <PageInfoButton
          :title="t('licenses.title')"
          :description="t('pageInfo.licenses')"
        />
      </div>
      <Button
        icon="pi pi-refresh"
      ...
      <Column field="plan" :header="t('licenses.plan')" style="width: 130px">
        <template #body="{ data: row }: { data: License }">
          <span class="text-sm text-[var(--text-muted)]">{{
            row.plan ?? t("common.none")
          }}</span>
        </template>
      </Column>
      <Column
        field="status"
        :header="t('licenses.status')"
        style="width: 120px"
      >
        <template #body="{ data: row }: { data: License }">
        ...
        <template #body="{ data: row }: { data: License }">
          <div class="flex gap-1">
            ...
            <Button
              ...
              rounded
              size="small"
              v-tooltip.top="t('licenses.cancel')"
