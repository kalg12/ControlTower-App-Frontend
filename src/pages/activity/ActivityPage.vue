...
watch(locale, (loc) => dayjs.locale(loc === "es" ? "es" : "en"), {
  immediate: true,
});
...
watch([dateFrom, dateTo, activeTab], () => {
  page.value = 0;
});
...
const eventTypeFilter = computed<ActivityEventType | undefined>(() =>
  activeTab.value === "all"
    ? undefined
    : (activeTab.value.toUpperCase() as ActivityEventType),
);
...
const todayUniqueUsers = computed(
  () => new Set(todayLogs.value.map((l) => l.userId)).size,
);
const todayActionCount = computed(
  () => todayLogs.value.filter((l) => l.eventType === "ACTION").length,
);
...
function getMeta(log: UserActivity): ActionMeta {
  ...
  return {
    ...base,
    label: t(labelKey, log.actionName?.replace(/_/g, " ") ?? "Action"),
  };
}
...
<template>
  <div class="space-y-6">
    ...
    <div class="flex items-center justify-between gap-4">
      <div class="flex items-center gap-2">
        ...
        <PageInfoButton
          ...
          :description="t('pageInfo.activity')"
        />
      </div>
      <Button
        icon="pi pi-refresh"
    ...
    <div class="grid grid-cols-2 lg:grid-cols-4 gap-3">
      <!-- Active now -->
      <div
        class="bg-[var(--surface)] border border-[var(--border)] rounded-[var(--radius)] p-4"
      >
        <div class="flex items-start justify-between mb-3">
          ...
          <div
            class="w-8 h-8 rounded-lg bg-green-50 dark:bg-green-950/60 flex items-center justify-center"
          >
            <span class="w-2 h-2 rounded-full bg-green-500 animate-pulse" />
          </div>
        </div>
        <p class="text-2xl font-bold text-[var(--text)]">
          {{ activeUsersCount ?? liveUsers.length }}
        </p>
        <p class="text-xs text-[var(--text-muted)] mt-1">
          {{ t("activity.last15min") }}
        </p>
      </div>
      ...
      >
        <div class="flex items-start justify-between mb-3">
          <p
            class="text-xs font-medium text-[var(--text-muted)] uppercase tracking-wide"
          >
            {{ t("activity.eventsToday") }}
          </p>
          <div
            class="w-8 h-8 rounded-lg bg-blue-50 dark:bg-blue-950/60 flex items-center justify-center"
          >
        ...
        <p class="text-xs text-[var(--text-muted)] mt-1">
          {{ todayActionCount }} {{ t("activity.actions") }}
        </p>
      </div>
      ...
      >
        <div class="flex items-start justify-between mb-3">
          <p
            class="text-xs font-medium text-[var(--text-muted)] uppercase tracking-wide"
          >
            {{ t("activity.uniqueUsersToday") }}
          </p>
          <div
            class="w-8 h-8 rounded-lg bg-violet-50 dark:bg-violet-950/60 flex items-center justify-center"
          >
      ...
      >
        <div class="flex items-start justify-between mb-3">
          ...
          <div
            class="w-8 h-8 rounded-lg bg-amber-50 dark:bg-amber-950/60 flex items-center justify-center"
          >
    ...
    <div class="flex flex-wrap items-center gap-2">
      <!-- Tabs -->
      <div
        class="flex items-center bg-[var(--surface)] border border-[var(--border)] rounded-lg p-0.5 gap-0.5"
      >
        <button
    ...
    <div class="flex gap-6 items-start">
      ...
      <div class="flex-1 min-w-0">
        ...
        <div v-if="isLoading && !result" class="space-y-0">
          <div v-for="i in 12" :key="i" class="flex gap-3 py-3 animate-pulse">
            <div
              class="w-8 h-8 rounded-full bg-[var(--surface-hover)] shrink-0"
            />
        ...
        >
          <div
            class="w-14 h-14 rounded-full bg-[var(--surface-hover)] flex items-center justify-center mb-4"
          >
          ...
          <p
            v-if="hasActiveFilters"
            class="text-xs text-[var(--text-muted)] mt-1 max-w-xs"
          >
            {{ t("activity.noDataFiltered") }}
          </p>
        </div>
        ...
        <div v-else class="space-y-8">
          <div v-for="group in groupedLogs" :key="group.key">
            ...
            <div class="flex items-center gap-3 mb-3">
              <span
                class="text-xs font-semibold text-[var(--text-muted)] uppercase tracking-widest"
                >{{ group.label }}</span
              >
            ...
            <div class="relative">
              ...
              <div
                class="absolute left-[15px] top-4 bottom-4 w-px bg-[var(--border)]"
              />
                ...
                >
                  ...
                  <div class="flex-1 min-w-0">
                    ...
                    <div class="flex items-center gap-3 mt-1">
                      <span
                        v-if="
                          log.eventType === 'NAVIGATION' && log.durationSeconds
                        "
                  ...
                  <!-- Timestamp -->
                  <time
                    class="text-xs text-[var(--text-muted)] shrink-0 mt-1 tabular-nums opacity-0 group-hover:opacity-100 transition-opacity"
                  >
          ...
          <!-- Pagination -->
          <div
            class="flex items-center justify-between pt-2 border-t border-[var(--border)]"
          >
      ...
      <!-- ── Sidebar ────────────────────────────────────────────── -->
      <aside
        class="hidden lg:flex flex-col gap-4 w-60 xl:w-64 shrink-0 sticky top-4"
      >
        <!-- Live right now -->
        <Card>
          <template #header>
            <span class="text-sm font-semibold text-[var(--text)]">{{
              t("activity.liveUsers")
            }}</span>
          ...
          <div
            ...
            class="text-xs text-[var(--text-muted)] text-center py-4"
          >
            {{ t("activity.noLiveUsers") }}
          </div>
          <div v-else class="space-y-3">
            <div
              v-for="u in liveUsers.slice(0, 6)"
              :key="u.email"
            ...
            >
              <div class="relative shrink-0">
                ...
                <span
                  class="absolute -bottom-0.5 -right-0.5 w-2.5 h-2.5 rounded-full bg-green-500 border-2 border-[var(--surface)]"
                />
              </div>
              <div class="min-w-0">
                <p
                  class="text-xs font-medium text-[var(--text)] truncate leading-tight"
                >
                  {{ u.name }}
                </p>
          ...
          <div v-else class="space-y-3">
            <div v-for="p in topPages" :key="p.label">
              <div class="flex justify-between items-baseline text-xs mb-1">
                <span
                  class="text-[var(--text)] truncate max-w-[150px] xl:max-w-[170px]"
                  >{{ p.label }}</span
                >
                <span
                  class="text-[var(--text-muted)] tabular-nums ml-2 shrink-0"
                  >{{ p.count }}</span
                >
              </div>
              <div class="h-1 rounded-full bg-[var(--border)]">
                <div
                  ...
                  :style="{
                    width: `${Math.round((p.count / maxPageCount) * 100)}%`,
                  }"
                />
              </div>
            </div>
        ...
        <Card>
          ...
          <div v-else class="space-y-3">
            ...
            >
              <span
                class="text-xs font-bold text-[var(--text-muted)] w-4 shrink-0 tabular-nums"
                >{{ i + 1 }}</span
              >
              <AppAvatar :name="u.name" size="sm" class="shrink-0" />
              <span class="text-xs text-[var(--text)] flex-1 truncate">{{
                u.name
              }}</span>
              <span
                class="text-xs font-semibold text-[var(--text-muted)] shrink-0 tabular-nums"
                >{{ u.count }}</span
              >
            </div>
          </div>
