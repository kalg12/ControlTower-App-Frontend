...
function confirmToggle(ep: Integration) {
  ...
  confirm.require({
    message: activating
      ? t("integrations.activateConfirm", {
          name: ep.name || ep.pullUrl || ep.id,
        })
      : t("integrations.deactivateConfirm", {
          name: ep.name || ep.pullUrl || ep.id,
        }),
    header: activating
      ? t("integrations.activateTitle")
      : t("integrations.deactivateTitle"),
    ...
    acceptProps: {
      ...
      severity: activating ? "success" : "warn",
    },
    accept: async () => {
      try {
...
const [regVersion, regVersionAttrs] =
  registerForm.defineField("contractVersion");
...
async function regenerateKey() {
  ...
  try {
    newApiKey.value = await integrationsService.regenerateApiKey(
      editingId.value,
    );
  } catch {
...
const events = computed<IntegrationEvent[]>(
  () => eventsResult.value?.content ?? [],
);
const webhooks = computed<WebhookDelivery[]>(
  () => webhooksResult.value?.content ?? [],
);
...
<template>
  <div class="space-y-6">
    ...
    <div class="flex items-start justify-between gap-4">
      <div class="flex items-center gap-2">
        ...
        <PageInfoButton
          ...
          :description="t('pageInfo.integrations')"
        />
      </div>
    ...
    >
      <div
        class="rounded-xl border border-[var(--border)] bg-[var(--surface)] px-4 py-3"
      >
        <p class="text-xs text-[var(--text-muted)] mb-1">
          {{ t("integrations.totalLabel") }}
        </p>
    ...
    <!-- Filter tabs -->
    <div
      v-if="!isLoading && !isError && integrations.length > 0"
      class="flex items-center gap-1 p-1 rounded-lg bg-[var(--surface-alt,var(--surface))] border border-[var(--border)] w-fit"
    >
      <button
    ...
    <EmptyState
      ...
      :action-label="
        activeFilter === 'ALL' ? t('integrations.registerEndpoint') : undefined
      "
    ...
    >
      ...
      <Column
        ...
        :header="t('integrations.interval')"
        style="width: 110px"
      >
        <template #body="{ data: row }: { data: Integration }">
          <span class="text-sm text-[var(--text-muted)]">
            {{
              t("integrations.heartbeatEvery", {
                interval: row.heartbeatIntervalSeconds,
              })
            }}
          </span>
        </template>
  ...
  >
    <form class="flex flex-col gap-4" @submit.prevent="onRegisterSubmit">
      <FormField
        ...
        name="reg-name"
        :error="registerForm.errors.value.name"
      >
        <InputText
          v-model="regName"
          v-bind="regNameAttrs"
      ...
      <FormField
        ...
        name="reg-type"
        :error="registerForm.errors.value.type"
        required
      >
        <Select
          v-model="regType"
      ...
      <FormField
        :label="t('integrations.pullUrl')"
        name="reg-url"
        :error="registerForm.errors.value.pullUrl"
      >
        <InputText
          v-model="regUrl"
          v-bind="regUrlAttrs"
      ...
      <FormField
        ...
        name="reg-key"
        :error="registerForm.errors.value.apiKey"
      >
        <InputText
          v-model="regKey"
          v-bind="regKeyAttrs"
      ...
      <FormField
        :label="t('integrations.interval')"
        name="reg-interval"
        :error="registerForm.errors.value.heartbeatIntervalSeconds"
      >
        <InputNumber
          v-model="regInterval"
          :min="30"
        ...
        <p class="text-xs text-[var(--text-muted)] mt-1">
          {{ t("integrations.intervalHint") }}
        </p>
      </FormField>
      <FormField
        :label="t('integrations.contractVersion')"
        name="reg-version"
        :error="registerForm.errors.value.contractVersion"
      >
        <InputText
          v-model="regVersion"
  ...
  >
    <form class="flex flex-col gap-4" @submit.prevent="onEditSubmit">
      ...
      <FormField
        :label="t('integrations.pullUrl')"
        name="edit-url"
        :error="editForm.errors.value.pullUrl"
      >
        <InputText
          v-model="editUrl"
          v-bind="editUrlAttrs"
      ...
      <FormField
        :label="t('integrations.apiKey')"
        name="edit-key"
        :error="editForm.errors.value.apiKey"
      >
        <InputText
          v-model="editKey"
          v-bind="editKeyAttrs"
      ...
      <FormField
        :label="t('integrations.interval')"
        name="edit-interval"
        :error="editForm.errors.value.heartbeatIntervalSeconds"
      >
        <InputNumber
      ...
      <FormField
        :label="t('integrations.contractVersion')"
        name="edit-version"
        :error="editForm.errors.value.contractVersion"
      >
        <InputText
          v-model="editVersion"
      ...
      <!-- Regenerate API Key -->
      <div
        class="rounded-lg border border-[var(--border)] bg-[var(--surface-alt,var(--surface))] p-3 space-y-2"
      >
        <p class="text-xs font-medium text-[var(--text-muted)]">
          {{ t("integrations.regenerateKeySection") }}
        </p>
        <Button
        ...
        <div
          v-if="newApiKey"
          class="mt-2 rounded-lg border border-amber-300 bg-amber-50 dark:bg-amber-950/30 p-3 space-y-2"
        >
          <p class="text-xs text-amber-700 dark:text-amber-400 font-medium">
            {{ t("integrations.newKeyWarning") }}
          </p>
          <div
            class="flex items-center gap-2 font-mono text-xs bg-white dark:bg-zinc-900 border border-[var(--border)] rounded px-3 py-2"
          >
            <span class="flex-1 break-all">{{ newApiKey }}</span>
            <button
              class="shrink-0 p-1 rounded hover:bg-[var(--border)]"
              @click.prevent="copyNewKey"
            >
  ...
  >
    <div v-if="drawerEndpoint" class="space-y-4">
      <!-- Endpoint meta -->
      <div
        class="rounded-lg border border-[var(--border)] bg-[var(--surface)] p-4 space-y-1"
      >
        <div class="flex items-center gap-2">
          <Tag
            :value="drawerEndpoint.type"
            :severity="drawerEndpoint.type === 'POS' ? 'info' : 'secondary'"
          />
          <Tag
            :severity="drawerEndpoint.active ? 'success' : 'secondary'"
            :value="
              drawerEndpoint.active
                ? t('integrations.activeLabel')
                : t('integrations.inactiveLabel')
            "
          />
        ...
        <p
          v-if="drawerEndpoint.clientName"
          class="text-sm text-[var(--text-muted)]"
        >
          {{ drawerEndpoint.clientName }} <span class="opacity-60">›</span>
          {{ drawerEndpoint.branchName }}
        </p>
        <p
          v-if="drawerEndpoint.pullUrl"
          class="text-xs font-mono text-[var(--text-muted)] truncate"
        >
          {{ drawerEndpoint.pullUrl }}
        </p>
      </div>
            ...
            <div v-else class="space-y-2 mt-2">
              ...
              >
                <div class="flex items-center justify-between gap-2">
                  <span
                    class="text-xs font-medium font-mono text-[var(--text)]"
                    >{{ ev.eventType }}</span
                  >
                  <Tag
                    :value="
              ...
              <div class="flex justify-between items-center pt-1">
                ...
                <span class="text-xs text-[var(--text-muted)]"
                  >{{ t("common.page") }} {{ eventsPage + 1 }}</span
                >
                <Button
                  text
                  size="small"
                  :disabled="eventsResult?.last ?? true"
                  @click="eventsPage++"
          ...
          <TabPanel value="1">
            ...
            <div v-else class="space-y-2 mt-2">
              ...
              >
                <div class="flex items-center justify-between gap-2">
                  <span
                    class="text-xs font-mono text-[var(--text-muted)] truncate max-w-[260px]"
                    :title="wh.url"
                    >{{ wh.url }}</span
                  >
                  <Tag
                    :value="wh.status"
                    :severity="webhookSeverity(wh.status)"
                ...
                >
                  <span
                    >{{ t("integrations.attempts") }}: {{ wh.attempts }}</span
                  >
                  <span v-if="wh.responseStatus"
                    >HTTP {{ wh.responseStatus }}</span
                  >
              ...
              <div class="flex justify-between items-center pt-1">
                ...
                >
                <span class="text-xs text-[var(--text-muted)]"
                  >{{ t("common.page") }} {{ webhooksPage + 1 }}</span
                >
                <Button
                  text
                  size="small"
                  :disabled="webhooksResult?.last ?? true"
                  @click="webhooksPage++"
                  >{{ t("common.next") }}</Button
                >
              </div>
