...
const clientIntegrations = computed<Integration[]>(() =>
  (integrationsResult.value?.content ?? []).filter(
    (i) => i.clientId === id.value,
  ),
);

function ticketStatusSeverity(
  status: string,
...
const onSubmitInteraction = interactionForm.handleSubmit(async (values) => {
  ...
  try {
    ...
    await queryClient.invalidateQueries({
      queryKey: ["interactions", id.value],
    });
    await queryClient.invalidateQueries({ queryKey: ["client", id.value] });
...
function openEditOpp(opp: ClientOpportunity) {
  ...
  oppForm.setValues({
    ...
    expectedCloseDate: opp.expectedCloseDate
      ? dayjs(opp.expectedCloseDate).format("YYYY-MM-DD")
      : "",
    source: opp.source ?? "",
...
const onSubmitOpp = oppForm.handleSubmit(async (values) => {
  ...
  try {
    ...
    await queryClient.invalidateQueries({
      queryKey: ["opportunities", id.value],
    });
    await queryClient.invalidateQueries({ queryKey: ["client", id.value] });
...
<template>
  <div class="space-y-6">
    ...
    <div class="flex items-center justify-between gap-4">
      <Button
        ...
        :label="t('tickets.backToTickets') || 'Back to Clients'"
        severity="secondary"
        text
    ...
    <template v-else-if="clientLoading">
      <div
        class="rounded-xl border border-[var(--border)] bg-[var(--surface)] p-6 space-y-3"
      >
      ...
      <div class="grid grid-cols-2 md:grid-cols-5 gap-4">
        <Card>
          <div class="flex items-center gap-3">
            <div
              class="w-9 h-9 rounded-lg bg-green-50 dark:bg-green-950 flex items-center justify-center"
            >
            ...
            <div>
              ...
              <p
                ...
                :class="healthScoreColor(client.healthScore)"
              >
                {{ client.healthScore ?? "—" }}
              </p>
            </div>
        ...
        <Card>
          <div class="flex items-center gap-3">
            <div
              class="w-9 h-9 rounded-lg bg-blue-50 dark:bg-blue-950 flex items-center justify-center"
            >
        ...
        <Card>
          <div class="flex items-center gap-3">
            <div
              class="w-9 h-9 rounded-lg bg-violet-50 dark:bg-violet-950 flex items-center justify-center"
            >
              <TrendingUp class="w-4.5 h-4.5 text-violet-500" />
            </div>
            <div>
              <p class="text-xs text-[var(--text-muted)]">
                {{ t("crm.openOpportunities") }}
              </p>
        ...
        <Card>
          <div class="flex items-center gap-3">
            <div
              class="w-9 h-9 rounded-lg bg-amber-50 dark:bg-amber-950 flex items-center justify-center"
            >
              <DollarSign class="w-4.5 h-4.5 text-amber-500" />
            </div>
            <div>
              <p class="text-xs text-[var(--text-muted)]">
                {{ t("crm.totalPipelineValue") }}
              </p>
        ...
        <Card>
          <div class="flex items-center gap-3">
            <div
              class="w-9 h-9 rounded-lg bg-emerald-50 dark:bg-emerald-950 flex items-center justify-center"
            >
      ...
      >
        <div class="flex items-start justify-between mb-4">
          <div>
            ...
            <p
              ...
              class="text-sm text-[var(--text-muted)] mt-0.5"
            >
              {{ client.legalName }}
            </p>
          </div>
        ...
        <div class="grid grid-cols-2 md:grid-cols-4 gap-4">
          <div>
            ...
            <p class="text-sm font-mono text-[var(--text)]">
              {{ client.taxId ?? "—" }}
            </p>
          </div>
          <div>
          ...
          <div v-if="client.website">
            <p
              class="text-xs text-[var(--text-muted)] uppercase tracking-wide mb-1"
            >
              {{ t("clientsPage.website") }}
            </p>
            <a
              :href="client.website"
              target="_blank"
              rel="noopener"
              class="text-sm text-[var(--primary)] hover:underline truncate block"
              >{{ client.website }}</a
            >
          </div>
        </div>
      ...
      <Tabs v-model:value="activeTab">
        <TabList class="flex gap-1 rounded-xl bg-[var(--surface)] p-1">
          ...
          <Tab value="integrations">{{
            t("integrations.tabIntegrations")
          }}</Tab>
        ...
        <TabPanels class="mt-4">
            ...
            <div class="grid grid-cols-1 lg:grid-cols-2 gap-6">
              ...
              <Card>
                ...
                <div
                  ...
                  class="text-center py-6 text-[var(--text-muted)]"
                >
                  {{ t("crm.noInteractions") }}
                </div>
                <div v-else class="space-y-2">
                  <div
                    v-for="inter in interactions.slice(0, 5)"
                    :key="inter.id"
                    class="flex items-start gap-3 px-3 py-2 rounded-lg bg-[var(--surface-raised)]"
                  >
                    <div
                    ...
                    <div class="min-w-0 flex-1">
                      ...
                      <p class="text-xs text-[var(--text-muted)]">
                        {{ interactionTypeLabel(inter.interactionType) }} ·
                        {{ inter.userName }} ·
                        {{ formatDateTime(inter.occurredAt) }}
                      </p>
                      <p
                        v-if="inter.outcome"
                        class="text-xs text-[var(--text-muted)] mt-0.5 truncate"
                      >
                        {{ inter.outcome }}
                      </p>
                    </div>
                ...
                <div v-else class="space-y-2">
                  ...
                  >
                    <div class="min-w-0 flex-1">
                      ...
                      <p class="text-xs text-[var(--text-muted)]">
                        {{ stageLabel(opp.stage as OpportunityStage) }} ·
                        {{ opp.ownerName || "—" }}
                      </p>
                    </div>
                    <div class="text-right ml-3 flex-shrink-0">
                      ...
                      <Tag
                        :severity="stageSeverity(opp.stage as OpportunityStage)"
                        :value="stageLabel(opp.stage as OpportunityStage)"
                ...
                <div class="flex items-center justify-between">
                  ...
                  <div class="flex gap-2">
                    <Button
                      ...
                      outlined
                      size="small"
                      @click="refetchInteractions()"
                    />
                    <Button
                      :label="t('crm.logInteraction')"
                      icon="pi pi-plus"
                      size="small"
                      @click="openLogInteraction()"
                    />
                  </div>
                </div>
              </template>
              ...
              >
                ...
                >
                  <template #body="{ data: row }: { data: ClientInteraction }">
                    <Tag
                      severity="info"
                      :value="interactionTypeLabel(row.interactionType)"
                    />
                  </template>
                </Column>
                <Column
                ...
                <Column :header="t('common.actions')" style="width: 80px">
                  <template #body="{ data: row }: { data: ClientInteraction }">
                    <Button
                      ...
                      rounded
                      size="small"
                      @click="confirmDeleteInteraction(row)"
                    />
                  </template>
                </Column>
                <template #empty>
            ...
            <div class="flex items-center justify-between mb-4">
              <h3 class="text-base font-semibold text-[var(--text)]">
                ...
                <span
                  ...
                  class="text-[var(--text-muted)] font-normal text-sm ml-1"
                  >({{ branches.length }})</span
                >
              </h3>
              <Button
                :label="t('clientsPage.addBranch')"
                icon="pi pi-plus"
                size="small"
              ...
              >
                <template #body="{ data: row }: { data: ClientBranch }">
                  <Tag
                    ...
                    :value="
                      row.isActive ? t('health.active') : t('health.inactive')
                    "
                  />
                </template>
              </Column>
          ...
          <TabPanel value="pipeline">
            <div class="flex items-center justify-between mb-4">
              <div>
                ...
                <p class="text-xs text-[var(--text-muted)]">
                  {{ t("crm.totalPipelineValue") }}:
                  {{ formatCurrency(pipelineValue) }}
                  <span v-if="wonRevenue > 0" class="ml-3"
                    >{{ t("crm.wonRevenue") }}:
                    {{ formatCurrency(wonRevenue) }}</span
                  >
                </p>
              </div>
              <div class="flex gap-2">
            ...
            >
              ...
              >
                <template #body="{ data: row }: { data: ClientOpportunity }">
                  <Tag
                    :severity="stageSeverity(row.stage as OpportunityStage)"
                    :value="stageLabel(row.stage as OpportunityStage)"
                  />
                </template>
              </Column>
              <Column
              ...
              <Column
                ...
                :header="t('crm.expectedCloseDate')"
                style="width: 150px"
              >
                <template #body="{ data: row }: { data: ClientOpportunity }">
                  <span class="text-sm text-[var(--text-muted)]">{{
            ...
            <div class="space-y-4 pt-4">
              ...
              <template v-else-if="financeSummary">
                <div class="grid grid-cols-2 sm:grid-cols-4 gap-4">
                  <div
                    class="bg-[var(--surface)] border border-[var(--border)] rounded-xl p-4"
                  >
                    <p
                      class="text-xs text-[var(--text-muted)] uppercase tracking-wide mb-1"
                    >
                      {{ t("finance.summary.totalInvoiced") }}
                    </p>
                    <p class="text-xl font-bold text-[var(--text)]">
                  ...
                  >
                    <p
                      class="text-xs text-[var(--text-muted)] uppercase tracking-wide mb-1"
                    >
                      {{ t("finance.summary.outstanding") }}
                    </p>
                    <p
                      class="text-xl font-bold"
                      :class="
                        financeSummary.totalOutstanding > 0
                          ? 'text-orange-500'
                          : 'text-[var(--text)]'
                      "
                    >
                      {{ fmtCurrency(financeSummary.totalOutstanding) }}
                    </p>
                  </div>
                  <div
                    class="bg-[var(--surface)] border border-[var(--border)] rounded-xl p-4"
                ...
                <div
                  v-if="financeSummary.lastInvoiceAt"
          ...
          <TabPanel value="integrations">
            <div class="space-y-3 pt-4">
              ...
              <div
                v-else-if="clientIntegrations.length === 0"
                class="text-center py-10 text-sm text-[var(--text-muted)]"
              >
                ...
                <div class="flex items-start justify-between gap-3">
                  <div class="space-y-1 min-w-0">
                    <div class="flex items-center gap-2">
                      ...
                      <Tag
                        :value="ep.type"
                        :severity="ep.type === 'POS' ? 'info' : 'secondary'"
                      />
                      <Tag
                        :severity="ep.active ? 'success' : 'secondary'"
                        :value="
                          ep.active
                            ? t('integrations.activeLabel')
                            : t('integrations.inactiveLabel')
                        "
                      />
                    </div>
                    <p
                    ...
                    <p
                      v-if="ep.pullUrl"
                      class="text-xs font-mono text-[var(--text-muted)] truncate max-w-xs"
                    >
                      {{ ep.pullUrl }}
                    </p>
                  </div>
          ...
          <TabPanel value="calendar">
            <div class="space-y-3 pt-4">
              ...
              <div
                v-else-if="!clientCalendarEvents?.length"
                class="text-center py-10 text-sm text-[var(--text-muted)]"
              >
                {{ t("calendar.noEvents") }}
              </div>
              ...
              >
                <div class="space-y-0.5 min-w-0">
                  <div class="flex items-center gap-2 flex-wrap">
                    ...
                    <Tag
                      :severity="
                        calStatusSeverity(event.status as CalendarEventStatus)
                      "
                      class="text-[10px]"
                    >
                      {{
            ...
            <Dialog
              ...
              :header="t('calendar.scheduleVisit')"
              modal
              ...
              <div class="flex flex-col gap-3 pt-2">
                ...
                <div class="grid grid-cols-2 gap-3">
                  <div class="flex flex-col gap-1">
                    ...
                    <Select
                      v-model="calForm.eventType"
                      :options="calendarEventTypeOpts"
                      option-label="label"
                      option-value="value"
                      class="w-full"
                    />
                  ...
                  <div class="flex flex-col gap-1">
                    ...
                    <Select
                      v-model="calForm.contactChannel"
                      :options="calendarChannelOpts"
                      option-label="label"
                      option-value="value"
                      class="w-full"
                      show-clear
                ...
                <div class="flex justify-end gap-2">
                  ...
                  <Button
                    :label="t('common.save')"
                    :loading="savingCalEvent"
                    :disabled="!calForm.title.trim()"
                    @click="saveCalEvent"
                  />
                </div>
              </div>
  ...
  <AppDialog
    ...
    :title="editingContact ? t('common.edit') : t('common.create')"
    :loading="isSubmittingContact"
  >
    <form class="flex flex-col gap-4" @submit.prevent="onSubmitContact">
      <FormField
        :label="t('clientsPage.contactFullName')"
        name="cf-fullName"
        :error="contactForm.errors.value.fullName"
        required
      >
        <InputText
          v-model="cfFullName"
          v-bind="cfFullNameAttrs"
          class="w-full"
      ...
      <div class="grid grid-cols-2 gap-3">
        <FormField
          :label="t('clientsPage.contactEmail')"
          name="cf-email"
          :error="contactForm.errors.value.email"
        >
          <InputText
            v-model="cfEmail"
            v-bind="cfEmailAttrs"
        ...
        <FormField
          :label="t('clientsPage.contactPhone')"
          name="cf-phone"
          :error="contactForm.errors.value.phone"
        >
          <InputText
            v-model="cfPhone"
            v-bind="cfPhoneAttrs"
      ...
      <FormField
        :label="t('clientsPage.contactRole')"
        name="cf-role"
        :error="contactForm.errors.value.role"
        required
      >
        <Select
          v-model="cfRole"
    ...
    <template #footer>
      <div class="flex justify-end gap-2">
        ...
        />
        <Button
          :label="editingContact ? t('common.save') : t('common.create')"
          :loading="isSubmittingContact"
  ...
  <AppDialog
    ...
    :title="t('clientsPage.addBranch')"
    :loading="isSubmittingBranch"
  >
    <form class="flex flex-col gap-4" @submit.prevent="onSubmitBranch">
      <FormField
        :label="t('clientsPage.branchName')"
        name="name"
        :error="branchForm.errors.value.name"
        required
      >
        <InputText
      ...
      <FormField
        ...
        name="address"
        :error="branchForm.errors.value.address"
      >
        <InputText
      ...
      <div class="grid grid-cols-2 gap-3">
        <FormField
          :label="t('clientsPage.city')"
          name="city"
          :error="branchForm.errors.value.city"
        >
          <InputText
            v-model="cityValue"
            v-bind="cityAttrs"
        ...
        <FormField
          :label="t('clientsPage.country')"
          name="country"
          :error="branchForm.errors.value.country"
          required
        >
          <InputText
  ...
  >
    <form class="flex flex-col gap-4" @submit.prevent="onEditBranchSubmit">
      <FormField
        :label="t('clientsPage.branchName')"
        name="eb-name"
        :error="editBranchForm.errors.value.name"
        required
      >
        <InputText
          v-model="ebName"
      ...
      <FormField
        :label="t('clientsPage.address')"
        name="eb-address"
        :error="editBranchForm.errors.value.address"
      >
        <InputText
          v-model="ebAddress"
          v-bind="ebAddressAttrs"
          class="w-full"
          :disabled="isEditingBranch"
        />
      </FormField>
      <FormField
        :label="t('clientsPage.city')"
        name="eb-city"
        :error="editBranchForm.errors.value.city"
      >
        <InputText
          v-model="ebCity"
          v-bind="ebCityAttrs"
  ...
  <AppDialog
    v-model:visible="showInteractionDialog"
    :title="t('crm.logInteraction')"
    :subtitle="t('crm.logInteractionSubtitle')"
    :loading="isSubmittingInteraction"
  >
    <form class="flex flex-col gap-4" @submit.prevent="onSubmitInteraction">
      <FormField
        :label="t('crm.title')"
        name="int-title"
        :error="interactionForm.errors.value.title"
        required
      >
        <InputText
          v-model="intTitle"
          v-bind="intTitleAttrs"
          class="w-full"
      ...
      <FormField
        :label="t('crm.type')"
        name="int-type"
        :error="interactionForm.errors.value.interactionType"
        required
      >
        <Select
          v-model="intType"
    ...
    <template #footer>
      <div class="flex justify-end gap-2">
        <Button
          ...
          outlined
          :disabled="isSubmittingInteraction"
          @click="showInteractionDialog = false"
        />
        <Button
          :label="t('common.save')"
          :loading="isSubmittingInteraction"
          @click="onSubmitInteraction"
        />
      </div>
    </template>
  </AppDialog>
  ...
  <AppDialog
    v-model:visible="showOppDialog"
    :title="editingOpp ? t('crm.editOpp') : t('crm.createOpp')"
    :subtitle="t('crm.createOppSubtitle')"
    :loading="isSubmittingOpp"
  >
    <form class="flex flex-col gap-4" @submit.prevent="onSubmitOpp">
      <FormField
        :label="t('crm.title')"
        name="opp-title"
        :error="oppForm.errors.value.title"
        required
      >
        <InputText
          v-model="oppTitle"
          v-bind="oppTitleAttrs"
          class="w-full"
      ...
      <div v-if="oppStage === 'CLOSED_LOST'" class="flex flex-col gap-2">
        <FormField
          :label="t('crm.lossReason')"
          name="opp-loss"
          :error="oppForm.errors.value.lossReason"
        >
          <Textarea
            v-model="oppLoss"
            v-bind="oppLossAttrs"
    ...
    <template #footer>
      <div class="flex justify-end gap-2">
        <Button
          ...
          outlined
          :disabled="isSubmittingOpp"
          @click="showOppDialog = false"
        />
        <Button
          :label="editingOpp ? t('common.save') : t('common.create')"
          :loading="isSubmittingOpp"
          @click="onSubmitOpp"
        />
      </div>
