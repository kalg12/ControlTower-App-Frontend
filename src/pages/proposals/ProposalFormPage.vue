...
const { data: clientsResult } = useQuery({
  ...
  queryFn: () =>
    import("@/services/clients.service").then((m) =>
      m.clientsService.list({ page: 0, size: 200 }),
    ),
  staleTime: 60000,
});
const clientOptions = computed(() =>
  (clientsResult.value?.content ?? []).map((c) => ({
    label: c.name,
    value: c.id,
  })),
);
...
watch(
  ...
  (p) => {
    ...
    discountType.value =
      (p.discountType as "PERCENTAGE" | "AMOUNT") ?? "PERCENTAGE";
...
function addLine() {
  lineItems.value.push({
    ...
    quantity: 1,
    unitPrice: 0,
    position: lineItems.value.length,
  });
}
...
async function downloadPdf() {
  ...
  const res = await api.get(`/proposals/${proposalId.value}/pdf`, {
    responseType: "blob",
  });
...
function buildPayload() {
  return {
    ...
    validityDate: validityDate.value
      ? dayjs(validityDate.value).format("YYYY-MM-DD")
      : undefined,
...
const updateMutation = useMutation({
  mutationFn: (data: ReturnType<typeof buildPayload>) =>
    proposalsService.update(proposalId.value!, data),
  onSuccess: (p) => {
    queryClient.invalidateQueries({ queryKey: ["proposals"] });
...
<template>
  <div class="p-6 max-w-4xl mx-auto space-y-6">
    ...
    <div
      class="grid grid-cols-1 md:grid-cols-2 gap-4 bg-white dark:bg-gray-800 rounded-xl border border-gray-200 dark:border-gray-700 p-6"
    >
      <div class="flex flex-col gap-1">
        <label class="text-sm font-medium text-gray-700 dark:text-gray-300"
          >{{ t("fields.client") }} <span class="text-red-500">*</span></label
        >
        <Select
          v-model="clientId"
      ...
      <div class="flex flex-col gap-1">
        <label class="text-sm font-medium text-gray-700 dark:text-gray-300"
          >{{ t("fields.title") }} <span class="text-red-500">*</span></label
        >
        <InputText
          v-model="title"
      ...
      <div class="flex flex-col gap-1 md:col-span-2">
        ...
        <Textarea
          ...
          rows="3"
          class="w-full"
          :placeholder="t('placeholders.proposalDescription')"
        />
      </div>
      <div class="flex flex-col gap-1">
        <label class="text-sm font-medium text-gray-700 dark:text-gray-300">{{
    ...
    <!-- Line Items -->
    <div
      class="bg-white dark:bg-gray-800 rounded-xl border border-gray-200 dark:border-gray-700 p-6 space-y-3"
    >
      ...
      <div class="overflow-x-auto">
        <table class="w-full text-sm">
          <thead>
            <tr
              class="text-left text-gray-500 border-b border-gray-200 dark:border-gray-700"
            >
          ...
          <tbody>
            ...
            >
              ...
              <td
                class="py-2 pr-3 text-right font-medium text-gray-700 dark:text-gray-300"
              >
                ${{
      ...
      <div class="flex flex-col sm:flex-row justify-between gap-6">
        ...
        <div class="w-64 space-y-1 text-sm self-end">
          ...
          <div class="flex justify-between text-gray-600 dark:text-gray-400">
            ...
            <span
              >${{
                taxAmount.toLocaleString(numberLocale, {
                  minimumFractionDigits: 2,
                })
              }}</span
            >
