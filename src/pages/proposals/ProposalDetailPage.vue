...
const sendMutation = useMutation({
  ...
  onSuccess: () => {
    invalidate();
    toast.success(t("proposals.emailSent"));
  },
  onError: (err: any) =>
    toast.error(err?.response?.data?.message ?? t("proposals.emailError")),
});
...
const viewedMutation = useMutation({
  ...
  onSuccess: () => {
    invalidate();
    toast.success(t("proposals.viewed"));
  },
...
function confirmSend() {
  confirm.require({
    message: t("proposals.sendConfirm", {
      client: proposal.value?.clientName ?? "",
    }),
    ...
    <template v-else-if="proposal">
      ...
      <div class="flex items-start justify-between gap-4 flex-wrap">
        <div class="flex items-center gap-3">
          ...
          <div>
            <div class="flex items-center gap-3">
              <h1
                class="text-xl font-bold text-gray-900 dark:text-white font-mono"
              >
                {{ proposal.number }}
              </h1>
              <Tag
              ...
              />
              <Tag
                v-if="proposal.emailViewedAt"
                severity="success"
                :value="`📧 ${t('proposals.emailOpened')}`"
                :title="
                  t('proposals.openedOn', {
                    date: formatDate(proposal.emailViewedAt),
                  })
                "
              />
            </div>
        ...
        <div class="flex flex-wrap gap-2">
          <template v-if="proposal.status === 'DRAFT'">
            <Button
              ...
              icon="pi pi-pencil"
              outlined
              @click="router.push(`/proposals/${id}/edit`)"
            />
            <Button
              :label="t('proposals.sendEmail')"
              icon="pi pi-send"
          ...
          <template v-if="proposal.status === 'SENT'">
            <Button
              :label="t('proposals.markViewed')"
              icon="pi pi-eye"
              outlined
              :loading="viewedMutation.isPending.value"
              @click="viewedMutation.mutate()"
            />
            <Button
              :label="t('proposals.acceptAction')"
              icon="pi pi-check"
              severity="success"
            ...
            />
            <Button
              :label="t('proposals.rejectAction')"
              icon="pi pi-times"
              severity="danger"
              outlined
          ...
          <Button
            ...
            icon="pi pi-download"
            outlined
            @click="downloadPdf(proposal.id, proposal.number)"
          />
        </div>
      </div>
          ...
          <p class="font-semibold text-gray-900 dark:text-white">
            {{
              proposal.validityDate
                ? dayjs(proposal.validityDate).format("DD MMM YYYY")
                : "—"
            }}
          </p>
        </div>
            ...
            {{
              proposal.acceptedAt
                ...
                : proposal.rejectedAt
                  ...
                  : t("proposals.response")
            }}
          </p>
          <p class="text-sm text-gray-700 dark:text-gray-300">
            {{ formatDate(proposal.acceptedAt ?? proposal.rejectedAt) }}
          </p>
        </div>
        <div
          v-if="proposal.emailViewedAt"
          class="bg-white dark:bg-gray-800 rounded-xl border border-green-200 dark:border-green-800 p-4"
        >
          <p class="text-xs text-green-600 uppercase tracking-wide mb-1">
            {{ t("proposals.emailOpened") }}
          </p>
          <p class="text-sm text-gray-700 dark:text-gray-300">
            {{ formatDate(proposal.emailViewedAt) }}
          </p>
        </div>
      </div>
      ...
      >
        ...
        >
          ...
          >
            <template #body="{ data }">{{
              formatCurrency(data.unitPrice, proposal.currency)
            }}</template>
          </Column>
          <Column
            field="subtotal"
            ...
            style="width: 140px"
            bodyClass="text-right font-medium"
          >
            <template #body="{ data }">{{
              formatCurrency(data.subtotal, proposal.currency)
            }}</template>
          </Column>
        </DataTable>
        <div class="flex justify-end">
          <div class="w-64 space-y-1 text-sm">
            ...
            <div
              v-if="proposal.discountAmount && proposal.discountAmount > 0"
            ...
            <div class="flex justify-between text-gray-500">
              ...
              ><span>{{
                formatCurrency(proposal.taxAmount, proposal.currency)
              }}</span>
            </div>
            <div
              class="flex justify-between font-bold text-gray-900 dark:text-white border-t border-gray-200 dark:border-gray-600 pt-1 text-base"
            >
