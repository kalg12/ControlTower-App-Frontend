...
const { data, isLoading, isError, refetch } = useQuery({
  queryKey: computed(() =>
    qk.templates(
      JSON.stringify({ q: debouncedQ.value, category: category.value }),
    ),
  ),
  queryFn: () =>
...
const form = ref<ResponseTemplateRequest>({
  name: "",
  body: "",
  category: "",
  shortcut: "",
...
const updateMutation = useMutation({
  mutationFn: ({ id, data }: { id: string; data: ResponseTemplateRequest }) =>
...
<template>
  <div class="space-y-6">
    <div class="flex items-center justify-between flex-wrap gap-3">
      <div>
        <h1
          class="text-2xl font-bold text-[var(--text)] flex items-center gap-2"
        >
    ...
    <div v-if="isLoading" class="grid sm:grid-cols-2 lg:grid-cols-3 gap-4">
      <div
        v-for="n in 6"
        :key="n"
        class="h-32 rounded-xl bg-[var(--surface-raised)] animate-pulse"
      />
    </div>
    ...
    <div v-else-if="templates.length === 0" class="py-20 text-center">
      <FileText
        class="w-12 h-12 mx-auto text-[var(--text-muted)] opacity-40 mb-3"
      />
    ...
    <div v-else class="grid sm:grid-cols-2 lg:grid-cols-3 gap-4">
      ...
      >
        <div class="flex items-start justify-between gap-2">
          <h3 class="font-semibold text-sm text-[var(--text)] truncate">
            {{ tpl.name }}
          </h3>
          <div class="flex gap-1 flex-shrink-0">
            <button
              class="p-1 rounded hover:bg-[var(--surface-raised)] text-[var(--text-muted)] hover:text-[var(--text)]"
              @click="openEdit(tpl)"
            >
              <Edit2 class="w-3.5 h-3.5" />
            </button>
            <button
              class="p-1 rounded hover:bg-red-50 dark:hover:bg-red-900/20 text-[var(--text-muted)] hover:text-red-500"
        ...
        >
          <span
            v-if="tpl.category"
            class="text-[10px] px-2 py-0.5 rounded-full bg-[var(--surface-raised)] text-[var(--text-muted)]"
            >{{ tpl.category }}</span
          >
          <span
            v-if="tpl.shortcut"
            class="flex items-center gap-1 text-[10px] text-[var(--primary)]"
          >
  ...
  <Dialog
    ...
    :header="editing ? t('templates.edit') : t('templates.new')"
    modal
  ...
  >
    <div class="space-y-4 pt-2">
      ...
      <div class="grid grid-cols-2 gap-4">
        <div>
          <label
            class="block text-xs font-medium text-[var(--text-muted)] mb-1"
            >{{ t("templates.category") }}</label
          >
          <InputText
        ...
        <div>
          <label
            class="block text-xs font-medium text-[var(--text-muted)] mb-1"
            >{{ t("templates.shortcut") }}</label
          >
          <InputText
            v-model="form.shortcut"
            class="w-full"
            :placeholder="t('templates.shortcutExample')"
          />
        </div>
      </div>
    </div>
