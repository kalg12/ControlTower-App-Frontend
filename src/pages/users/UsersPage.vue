...
const editSchema = z.object({
  ...
  password: z
    .string()
    .min(8, t("users.passwordMinIfSet"))
    .optional()
    .or(z.literal("")),
});
...
function openEditDialog(user: User) {
  ...
  editRoleIds.value = availableRoles.value
    .filter(
      (r) =>
        (user.roles as string[]).includes(r.id) ||
        (user.roles as string[]).includes(r.name),
    )
    .map((r) => r.id);
  editForm.setValues({
      ...
      <div class="flex items-center gap-2">
        <div>
          ...
          <p class="text-sm text-[var(--text-muted)]">
            {{ t("users.totalCount", { count: totalRecords }) }}
          </p>
        </div>
        <PageInfoButton
          :title="t('users.title')"
          :description="t('pageInfo.users')"
        />
      </div>
      ...
      <Column :header="t('users.roles')" style="min-width: 160px">
        <template #body="{ data: row }: { data: User }">
          <div class="flex flex-wrap gap-1">
            ...
            />
            <span
              v-if="!(row.roles as string[])?.length"
        ...
        <template #body="{ data: row }: { data: User }">
          <div class="flex items-center gap-1.5">
            <Shield
              ...
              v-tooltip.top="t('users.twoFactorEnabled')"
            />
            <ShieldOff
              v-else
              class="w-4 h-4 text-red-400"
              v-tooltip.top="t('users.twoFaDisabled')"
            />
          </div>
        </template>
      </Column>
  ...
  >
    <form class="flex flex-col gap-4" @submit.prevent="onInviteSubmit">
      ...
      <FormField
        ...
        name="inv-password"
        :error="inviteForm.errors.value.password"
        required
      >
        <InputText
          v-model="invPassword"
          v-bind="invPasswordAttrs"
          type="password"
  ...
  >
    <form class="flex flex-col gap-4" @submit.prevent="onEditSubmit">
      <FormField
        :label="t('users.fullName')"
        name="edit-name"
        :error="editForm.errors.value.fullName"
        required
      >
        <InputText
          v-model="editName"
      ...
      <FormField
        :label="t('users.email')"
        name="edit-email"
        :error="editForm.errors.value.email"
        required
      >
        <InputText
          v-model="editEmail"
