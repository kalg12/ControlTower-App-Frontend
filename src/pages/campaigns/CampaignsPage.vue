...
<template>
  <div class="space-y-4">
      ...
      <div class="flex items-center gap-2">
        <div>
          ...
          <p class="text-sm text-[var(--text-muted)]">
            {{ t("campaigns.totalCount", { count: totalRecords }) }}
          </p>
        </div>
        <PageInfoButton
          :title="t('campaigns.title')"
          :description="t('pageInfo.campaigns')"
        />
      </div>
    ...
    >
      ...
      >
        <template #body="{ data: row }: { data: Campaign }">
          <span
            class="text-sm text-[var(--text-muted)] truncate block max-w-xs"
            >{{ row.targetAudience ?? t("common.none") }}</span
          >
        </template>
      </Column>
      <Column
  ...
  <AppDialog
    ...
    :subtitle="t('campaigns.createSubtitle')"
  ...
  >
    <form class="flex flex-col gap-4" @submit.prevent="onCreateSubmit">
      ...
      <FormField
        ...
        name="c-type"
        :error="createForm.errors.value.type"
        required
      >
        <Select
          v-model="cType"
      ...
      <FormField
        :label="t('campaigns.formSubject')"
        name="c-subject"
        :error="createForm.errors.value.subject"
      >
        <InputText
          v-model="cSubject"
          v-bind="cSubjectAttrs"
          :placeholder="t('campaigns.formSubjectPlaceholder')"
          class="w-full"
          :disabled="isSubmitting"
        />
      </FormField>
      <FormField
        :label="t('campaigns.formBody')"
        name="c-body"
        :error="createForm.errors.value.body"
        required
      >
        <Textarea
          v-model="cBody"
          v-bind="cBodyAttrs"
          :placeholder="t('campaigns.formBodyPlaceholder')"
          :rows="4"
          class="w-full"
          :disabled="isSubmitting"
        />
      ...
      <FormField
        :label="t('campaigns.formAudience')"
        name="c-audience"
        :error="createForm.errors.value.targetAudience"
      >
        <InputText
          v-model="cAudience"
          v-bind="cAudienceAttrs"
          :placeholder="t('campaigns.formAudiencePlaceholder')"
          class="w-full"
          :disabled="isSubmitting"
        />
      </FormField>
